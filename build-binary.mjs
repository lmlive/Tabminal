...
async function embedPtyLib(target) {
  ...
  try {
    ...
    // Read the library file and convert to base64
    const libBuffer = await fs.readFile(sourceLibPath);
    const libBase64 = libBuffer.toString('base64');

    // Read the pty wrapper file
    const wrapperPath = 'src/utils/pty-wrapper.mjs';
    let wrapperContent = await fs.readFile(wrapperPath, 'utf8');

    // Replace the placeholder with the actual library data
    wrapperContent = wrapperContent.replace('__LIBRARY_DATA_PLACEHOLDER__', libBase64);

    // Write the modified wrapper back
    await fs.writeFile(wrapperPath, wrapperContent);

    console.log(`📦 Embedded ${libFileName} into pty wrapper`);
    return true;
  } catch (error) {
    console.warn(`⚠️  Warning: Could not embed ${libFileName}. This may cause runtime errors.`);
...
async function restorePtyLibPlaceholder() {
  try {
    ...
    wrapperContent = wrapperContent.replace(/__LIBRARY_DATA_PLACEHOLDER__.*'/, '__LIBRARY_DATA_PLACEHOLDER__');

    // Ensure the placeholder is properly restored
    if (!wrapperContent.includes('__LIBRARY_DATA_PLACEHOLDER__')) {
      // If the base64 string is very long, we need to replace the entire thing
      wrapperContent = wrapperContent.replace(/const LIBRARY_DATA_PLACEHOLDER = '[^']*';/, "const LIBRARY_DATA_PLACEHOLDER = '__LIBRARY_DATA_PLACEHOLDER__';");
    }

    await fs.writeFile(wrapperPath, wrapperContent);
    console.log('📦 Restored pty wrapper placeholder');
  } catch (error) {
    console.warn(`⚠️  Warning: Could not restore pty wrapper placeholder: ${error.message}`);
  }
}
